...
CameraSDK主要用于连接、设置和获取相机参数、控制相机进行拍照和录制、文件下载和固件升级(仅支持X4后续机型)等。仅支持通过usb进行连接相机。用于toB用户，主要支持的相机有ONE X，ONE X2、R/RS、X3、X4、X4 Air以及X5全景相机。主要支持的平台有Windows、Ubuntu22.04。
